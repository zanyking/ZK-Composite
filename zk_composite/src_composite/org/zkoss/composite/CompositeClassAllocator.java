 ...
 *
 */
public class CompositeClassAllocator implements ResourceAllocator<Class<? extends Component>> {
	...
	public void allocate(Resource resource, ResourceVisitor<Class<? extends Component>> visitor) {
		try {
			...
			try {
				...
				if(!hasCompositeAnnotation)return;
				
				if((clzNode.access & Opcodes.ACC_PUBLIC)==0)return;
				...
				try{
					...
					throw new ClassCastException(clz+" has \"Composite\" annotation but not a Component!");
				}
